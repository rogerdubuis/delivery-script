...
class MainWindow(QMainWindow):
    ...
    setpoint_load_finished = pyqtSignal(dict)
    ...
    def __init__(self):
        ...
        self.setpoint_load_finished.connect(self._handle_setpoint_load_finished)
    ...
    def _connect_worker(self, selected_port):
    ...
    def _load_current_setpoints_async(self):
        """Sync setpoint controls without blocking the Qt event loop."""
        threading.Thread(target=self._load_current_setpoints_worker, daemon=True).start()

    def _load_current_setpoints_worker(self):
        setpoints = {}
        if not self.serial_worker or not self.serial_worker.controller:
            self.setpoint_load_finished.emit(setpoints)
            return
        ...
        for channel_key in CHANNEL_KEYS:
            ...
            if success and isinstance(setpoint, (int, float)):
                setpoints[channel_key] = setpoint
        self.setpoint_load_finished.emit(setpoints)

    def _handle_setpoint_load_finished(self, setpoints):
        if not self.serial_worker or not self.serial_worker.controller:
            return

        for channel_key, setpoint in setpoints.items():
